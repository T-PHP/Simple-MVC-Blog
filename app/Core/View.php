...
class View
{
    ...
    /**
     * 
     * return absolute path to selected admintemplate directory
     * @param  string  $path  path to file from views folder
     * @param  array   $data  array of data
     * @param  string  $custom path to template folder
     */
    public static function renderAdminTemplate($path, $data = false, $custom = false)
    {
        if (!headers_sent()) {
            foreach (self::$headers as $header) {
                header($header, true);
        ...
        if ($custom == false) {
            ...
            require "app/templates/admin/$custom/$path.php";
        }
    }
